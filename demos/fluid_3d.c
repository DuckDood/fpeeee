...
SDL_AppResult SDL_AppInit(void **appstate, [[maybe_unused]] int argc, [[maybe_unused]] char **argv) {
	*appstate = malloc(sizeof(prog_state));
	...
	if(!SDL_CreateWindowAndRenderer("physics", WIDTH, HEIGHT, 0, &state->window, &state->renderer)) {
	...
	state->grid = construct_grid_3d(25, 100, 25, 0.2);
...
SDL_AppResult SDL_AppIterate(void *appstate) {
	...
	if(key_states[SDL_SCANCODE_LEFT]) {
		rotation = generate_rotation_matrix(0, 0.01, 0);
	} else {
		rotation = generate_rotation_matrix(0, 0, 0);
	}

	wall_3d bottom1 = (wall_3d) {
		.vertex_a = m3_v3_mult(rotation, (vec3){-2, 0, -2}),
		.vertex_b = m3_v3_mult(rotation, (vec3){-2, 0, 2}),
		.vertex_c = m3_v3_mult(rotation, (vec3){2, 0, 2}),
	};
	...
	wall_3d back1 = (wall_3d) {
		.vertex_a = m3_v3_mult(rotation, (vec3){-2, 0, 2}),
		.vertex_b = m3_v3_mult(rotation, (vec3){2, 0, 2}),
		.vertex_c = m3_v3_mult(rotation, (vec3){2, 1, 2}),
	};
	set_wall_normal(&back1);

	wall_3d back2 = (wall_3d) {
		.vertex_a = m3_v3_mult(rotation, (vec3){-2, 1, 2}),
		.vertex_b = m3_v3_mult(rotation, (vec3){2, 1, 2}),
		.vertex_c = m3_v3_mult(rotation, (vec3){-2, 0, 2}),
	};
	...
	for(int steps = 0; steps < steps_per_frame; ++steps) {
		for(int i = 0; i < state->ball_count; ++i) {
			...
			current_ball->position.y -= 9 * state->deltatime * state->deltatime;
		}
		for(int i = 0; i < state->ball_count; ++i) {
	...
	for(int i = 0; i < state->ball_count; ++i) {
		draw_circle_3d(state->renderer, state->balls[i], 10, state->cam);
	}
