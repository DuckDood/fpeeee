...
SDL_AppResult SDL_AppIterate(void *appstate) {
	...
	if(key_states[SDL_SCANCODE_RETURN]) {
		...
		for(int i = 0; i < parachute_size; ++i) {
			solve_constraint_2d(distance_constraint_2d,
					(vec2*[]){&state->balls[i].position, &state->balls[i+1].position}, 
					output_ptrs,
					(float[]){1/state->balls[i].mass, 1/state->balls[i+1].mass}, 
					2, (float[]){0.01}, state->deltatime, 0, INEQ_LESS);
		}
		solve_constraint_2d(distance_constraint_2d,
				(vec2*[]){&state->balls[0].position, &state->balls[parachute_size].position},
				output_ptrs,
				(float[]){1/state->balls[0].mass, 1/state->balls[parachute_size].mass}, 2
				, (float[]){parachute_size * 0.0125 * parascale}, state->deltatime, 0, EQUALITY);

		solve_constraint_2d(distance_constraint_2d,
				(vec2*[]){&state->balls[parachute_size].position, &state->balls[parachute_size+1].position},
				output_ptrs,
				(float[]){1/state->balls[parachute_size].mass, 1/state->balls[parachute_size+1].mass}
				, 2, (float[]){parachute_size * 0.01 * parascale}, state->deltatime, 0, EQUALITY);

		solve_constraint_2d(distance_constraint_2d,
				(vec2*[]){&state->balls[0].position, &state->balls[parachute_size+1].position},
				output_ptrs,
				(float[]){1/state->balls[0].mass, 1/state->balls[parachute_size+1].mass},
				2, (float[]){parachute_size * 0.01 * parascale}, state->deltatime, 0, EQUALITY);
