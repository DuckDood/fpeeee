...
SDL_AppResult SDL_AppIterate(void *appstate) {
	...
	if(key_states[SDL_SCANCODE_RETURN]) {
		if(SDL_GetTicks() > state->spawn_tick_count + state->spawn_delay) {
			...
			state->balls[state->ball_count-1].velocity = v2_fmult(velocity,10);
			//set_velocity_2d(&state->balls[state->ball_count-1], v2_fdiv(velocity, 1000));
		...
		for(int i = 0; i < parachute_size; ++i) {
			solve_constraint_2d(dist_constraint_2d,
					...
					(vec2*[]){&state->balls[i].position, &state->balls[i+1].position}, (float[]){1/state->balls[i].mass, 1/state->balls[i+1].mass}, 2, (float[]){0.01}, state->deltatime, 0);
		}
		solve_constraint_2d(dist_constraint_2d,
				(del_constraint_function_2d[]){dist_constraint_del_a_2d, dist_constraint_del_b_2d}, EQUALITY, 
				(vec2*[]){&state->balls[0].position, &state->balls[parachute_size].position}, (float[]){1/state->balls[0].mass, 1/state->balls[parachute_size].mass}, 2, (float[]){parachute_size * 0.0125 * parascale}, state->deltatime, 0);

		solve_constraint_2d(dist_constraint_2d,
				(del_constraint_function_2d[]){dist_constraint_del_a_2d, dist_constraint_del_b_2d}, EQUALITY, 
				(vec2*[]){&state->balls[parachute_size].position, &state->balls[parachute_size+1].position}, (float[]){1/state->balls[parachute_size].mass, 1/state->balls[parachute_size+1].mass}, 2, (float[]){parachute_size * 0.01 * parascale}, state->deltatime, 0);

		solve_constraint_2d(dist_constraint_2d,
				(del_constraint_function_2d[]){dist_constraint_del_a_2d, dist_constraint_del_b_2d}, EQUALITY, 
				(vec2*[]){&state->balls[0].position, &state->balls[parachute_size+1].position}, (float[]){1/state->balls[0].mass, 1/state->balls[parachute_size+1].mass}, 2, (float[]){parachute_size * 0.01 * parascale}, state->deltatime, 0);
