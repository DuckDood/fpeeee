...
spatial_grid_2d construct_grid_2d(int grid_width, int grid_height, int partition_max_particles, float element_size) {
	...
	for(int i = 0; i < grid_width * grid_height; ++i) {
		grid.partitions[i].ball_list = malloc(sizeof(ball_2d*) * partition_max_particles); // this is terrifying for memory usage i will need to convert this to a sparse grid at some point
		grid.partitions[i].ball_list_count = 0;
	}

	printf("ludicrous memory usage by grid (kb): %zu\n", sizeof(ball_2d*) * partition_max_particles * grid_width * grid_height / 1000);
...
void update_grid_2d(spatial_grid_2d *grid, ball_2d *balls, int ball_count) {
	...
	for(int i = 0; i < ball_count; ++i) {
		for(int row = 0; row < grid->grid_height; ++row) {
			for(int column = 0; column < grid->grid_width; ++column) {
				if(point_aabb_collision(balls[i].position, (vec2){grid->element_size * (column - grid->grid_width/2.), grid->element_size * (row - grid->grid_height/2.)}, (vec2){grid->element_size, grid->element_size})) {
					grid->partitions[row * grid->grid_width + column].ball_list[grid->partitions[row * grid->grid_width + column].ball_list_count++] = balls + i;
				}
				it_count++;
			}
		}
...
void spatial_collision_2d(spatial_grid_2d *grid, ball_2d *balls, int ball_count) {
	...
	for(int row = 0; row < grid->grid_height; ++row) {
		for(int column = 0; column < grid->grid_width; ++column) {
			spatial_partition_2d *current_partition = &grid->partitions[row * grid->grid_width + column];
			for(int i = 0; i < current_partition->ball_list_count; ++i) {
				for(int check_row = -1; check_row < 2; ++check_row) {
					for(int check_column = -1; check_column < 2; ++check_column) {
						if(row + check_row < 0) continue;
						if(column + check_column < 0) continue;
						if(row + check_row > grid->grid_height-1) continue;
						if(column + check_column > grid->grid_width-1) continue;
						spatial_partition_2d *current_check_partition = &grid->partitions[(row + check_row) * grid->grid_width + column + check_column];
						for(int j = 0; j < current_check_partition->ball_list_count; ++j) {
							if(current_partition->ball_list[i] == current_check_partition->ball_list[j]) continue;
							check_and_resolve_balls_2d(current_partition->ball_list[i], current_check_partition->ball_list[j]);
						}
					}
				}
			}
