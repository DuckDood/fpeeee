	...
	for(int i = 0; i < ball_count; ++i) {
		for(int row = 0; row < grid->height; ++row) {
			for(int column = 0; column < grid->width; ++column) {
				if(point_aabb_collision(balls[i].position, (vec2){grid->element_size * (column - grid->width/2.), grid->element_size * (row - grid->height/2.)}, (vec2){grid->element_size, grid->element_size})) {
					grid->partitions[row * grid->width + column].ball_list[grid->partitions[row * grid->width + column].ball_count++] = balls + i;
				}
				it_count++;
			}
		}
...
void spatial_collision_2d(spatial_grid_2d *grid, ball_2d *balls, int ball_count) {
	for(int row = 0; row < grid->height; ++row) {
		for(int column = 0; column < grid->width; ++column) {
			spatial_partition_2d *current_partition = &grid->partitions[row * grid->width + column];
			for(int i = 0; i < current_partition->ball_count; ++i) {
				for(int check_row = -1; check_row < 2; ++check_row) {
					for(int check_column = -1; check_column < 2; ++check_column) {
						...
						if(row + check_row > grid->height-1) continue;
						if(column + check_column > grid->width-1) continue;
						spatial_partition_2d *current_check_partition = &grid->partitions[(row + check_row) * grid->width + column + check_column];
						for(int j = 0; j < current_check_partition->ball_count; ++j) {
